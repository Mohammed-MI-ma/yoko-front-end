...
//__Save_Preferences__Product
export const savePreferencesProduct = createAsyncThunk(
  "auth/savePreferencesProduct",
  async ({ flag, t }, { rejectWithValue }) => {
    const config = {
      headers: {
        "Content-Type": "application/json",
      },
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/savePreferencesProduct`,
        { flag },
        config
      );
      if (response.status === 200) {
        message.success(t("savePreferencesSuccess"));
        return response.data;
      } else {
        message.error(t("unexpectedError"));
      }
...
//__Save_Preferences__Orders
export const savePreferencesOrders = createAsyncThunk(
  "auth/savePreferencesOrders",
  async ({ flag, t }, { rejectWithValue }) => {
    const config = {
      headers: {
        "Content-Type": "application/json",
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/savePreferencesOrders`,
        { flag },
        config
      );
      if (response.status === 200) {
